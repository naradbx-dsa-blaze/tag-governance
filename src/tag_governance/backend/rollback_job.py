...
def run():
    ...
    queue_table = _param("queue_table")  # optional: mark rolled-back rows re-taggable
...
def _mark_rolled_back(queue_table, batch_id, pairs):
    """Set status=ROLLED_BACK for the (workload_id, tag_key) rows of this batch
    that were successfully restored. Uses a temp view + MERGE so it's one atomic op."""
    from pyspark.sql.types import StructType, StructField, StringType
    schema = StructType([StructField("workload_id", StringType()),
                         StructField("tag_key", StringType())])
    spark.createDataFrame([{"workload_id": w, "tag_key": k} for w, k in pairs], schema) \
         .dropDuplicates().createOrReplaceTempView("_tg_rb")
    b = batch_id.replace("'", "''")
    spark.sql(f"""
        MERGE INTO {queue_table} t
        USING _tg_rb s
        ON t.batch_id = '{b}' AND t.workload_id = s.workload_id
           AND t.tag_key = s.tag_key AND t.status = 'SUCCEEDED'
        WHEN MATCHED THEN UPDATE SET t.status = 'ROLLED_BACK',
...
def _persist_rollback_audit(audit_table, audit_rows):
    # Explicit StringType schema: error/old_value/new_value can be all-None on a
    # clean rollback, and createDataFrame can't infer a type from an all-null
    # column (CANNOT_DETERMINE_TYPE) — which would crash AFTER tags were restored.
    from pyspark.sql import functions as F
    from pyspark.sql.types import StructType, StructField, StringType
    cols = ["audit_id", "batch_id", "executed_by", "workspace_id", "product",
            "workload_id", "tag_key", "old_value", "new_value", "action",
            "status", "error"]
    schema = StructType([StructField(c, StringType(), True) for c in cols])
    adf = spark.createDataFrame(
        [{c: r.get(c) for c in cols} for r in audit_rows], schema
